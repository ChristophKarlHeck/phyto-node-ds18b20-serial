...
*     @date 20/06/2011
*/
uint8_t DS18X20_get_power_status(uint8_t id[], onewireclass Tx) {
    uint8_t pstat;
    Tx.ow_reset();
    Tx.ow_command(DS18X20_READ_POWER_SUPPLY, id);
    pstat=Tx.ow_bit_io(1); // pstat 0=is parasite/ !=0 ext. powered
    Tx.ow_reset();
...
*     @date 20/06/2011
*/
uint8_t DS18X20_start_meas( uint8_t with_power_extern, uint8_t id[], onewireclass Tx) {
    Tx.ow_reset(); //**
    if ( Tx.ow_test_pin() ) { // only send if bus is "idle" = high
        Tx.ow_command( DS18X20_CONVERT_T, id );
        if (with_power_extern != DS18X20_POWER_EXTERN)
            Tx.ow_parasite_enable();
        return DS18X20_OK;
    }
...
uint8_t DS18X20_read_meas(uint8_t id[], uint8_t *subzero,
                          uint8_t *cel, uint8_t *cel_frac_bits, onewireclass Tx) {
    uint8_t i;
    uint8_t sp[DS18X20_SP_SIZE];

    Tx.ow_reset();
    Tx.ow_command(DS18X20_READ, id);
