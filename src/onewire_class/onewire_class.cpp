...
#include "crc8/crc8.h"
#include "onewire_class/onewire_class.h"

//DigitalInOut ow_pin(PA_3);

onewireclass::onewireclass(PinName targetPin): ow_pin(targetPin){}
//onewireclass::onewireclass(char Pin){
//    onewireclass::ow_Pin = new DigitalInOut(0x00);
//}
/**
*     @brief PUL-UP bus OW
*    @return OW_OK
*     @date 20/06/2011
*/
uint8_t onewireclass::ow_PullUp(void)
{
...
*     @date 20/06/2011
*/
uint8_t onewireclass::search_sensors(uint8_t *nSensors,uint8_t *gSensorIDs ) {
    uint8_t i;
...
*     @date 20/06/2011
*/
uint8_t onewireclass::ow_show_id( uint8_t id[], size_t n ,char *text) {
    size_t i;
    char hex[4];
    sprintf(text,"");
    ...
    for ( i = 0; i < n; i++ ) {
        ...
        //if ( i == n-1 ) strcat(text, "CRC: " );
        //if ( i == 1 ) strcat(text, " SN: " );
        sprintf(hex,"%2.2X ",id[i]);
        //strcat(text,hex);  <--- overwrited id?!
        if ( i == 0 ) {
...
*     @date 20/06/2011
*/
uint8_t onewireclass::ow_test_pin (void){
    if (ow_pin)
        return 1;
return 0;   
}
...
*     @date 20/06/2011
*/
uint8_t onewireclass::ow_reset(void) { // reset.  Should improve to act as a presence pulse
    uint8_t err;
...
*     @date 20/06/2011
*/
uint8_t onewireclass::ow_bit_io( uint8_t b ) {
...
*     @date 20/06/2011
*/
uint8_t onewireclass::ow_byte_wr( uint8_t b ) {
    uint8_t i = 8, j;
...
*     @date 20/06/2011
*/
uint8_t onewireclass::ow_byte_rd( void ) {
...
*     @date 20/06/2011
*/
uint8_t onewireclass::ow_rom_search( uint8_t diff, uint8_t id[] ) {
...
*     @date 20/06/2011
*/
uint8_t onewireclass::ow_command( uint8_t command, uint8_t id[] ) {
    uint8_t i;
...
*     @date 20/06/2011
*/
uint8_t onewireclass::ow_parasite_enable(void) {
    ow_pin.output();
    ow_pin = 1;
    return 0;
}
...
*     @date 20/06/2011
*/
uint8_t onewireclass::ow_parasite_disable(void) {
...
*/
uint8_t onewireclass::ow_find_sensor(uint8_t *diff, uint8_t id[]) {
...
*    @return  OW_OK 
*/
uint8_t onewireclass::OneWireOutByte(uint8_t d) { // output byte d (least sig bit first).
    for (int n=8; n!=0; n--) {
...
*/
uint8_t onewireclass::OneWireInByte(void) { // read byte, least sig byte first
    uint8_t d = 0, b;
    for (int n=0; n<8; n++) {
        ow_pin.output();
        ow_pin = 0;
